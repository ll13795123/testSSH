 ...
 */
package com.morning.hdfs_examples.pi;

import com.google.common.base.Charsets;
import com.morning.hdfs_examples.pi.math.Bellard;
...
public final class Parser {
  ...
  /** Parse a line */
  private static void parseLine(final String line, Map<Bellard.Parameter, List<TaskResult>> m) {
//      LOG.info("line = " + line);
    final Map.Entry<String, TaskResult> e = DistSum.string2TaskResult(line);
    if (e != null) {
      final List<TaskResult> sums = m.get(Bellard.Parameter.get(e.getKey()));
      if (sums == null)
  ...
  static <T extends Combinable<T>> Map<Bellard.Parameter, T> combine(Map<Bellard.Parameter, List<T>> m) {
    final Map<Bellard.Parameter, T> combined = new TreeMap<Bellard.Parameter, T>();
    for(Bellard.Parameter p : Bellard.Parameter.values()) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    //read input
    final Map<Bellard.Parameter, List<TaskResult>> parsed = new Parser(true).parse(inputpath, outputdir);
    final Map<Bellard.Parameter, TaskResult> combined = combine(parsed);
    long duration = 0;
